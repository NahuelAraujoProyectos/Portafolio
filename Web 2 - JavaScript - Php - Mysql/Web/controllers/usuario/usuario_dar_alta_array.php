<?php
    ...
    //Validamos los ingresos por seguridad
        ...
        foreach ($_POST['datos'] as $clave => $valor) {
            $_POST['datos'][$clave] = LimpiarDatos::validarDato($conexion, $valor);
        }
    //

    //Verificamos si hay campos obligatorios vacíos (array)
        $campos_obligatorios=[];
        foreach ($_POST['datos'] as $clave => $valor) {
    ...
    //Verificamos si la edad es inferior a 16 años
        $fecha_nacimiento = $_POST['datos']['fecha_de_nacimiento'];
        $edad_minima = 16;
    ...
    //Verificamos si los datos son validos
        $valores_invalidos=[];
        //Verificamos email
        if (!filter_var($_POST['datos']['email'], FILTER_VALIDATE_EMAIL)) {
            $valores_invalidos[]='email';
        };
        //Verificamos teléfono
        if (!filter_var($_POST['datos']['teléfono'], FILTER_VALIDATE_INT)) {
    ...
    //Verificamos que el usuario y email no están en uso (array)
        $valores_repetidos=[];
        //Usuario
    ...
    //Preparamos inserción
        ...
        // Creamos sentencia para tabla user_data
            $sql_data = "INSERT INTO user_data(nombre, apellidos, email, teléfono, fecha_de_nacimiento, dirección, sexo) VALUES(
                '".$_POST['datos']['nombre']."',
                '".$_POST['datos']['apellidos']."',
                '".$_POST['datos']['email']."',
                '".$_POST['datos']['teléfono']."',
                '".$_POST['datos']['fecha_de_nacimiento']."',
                '".$_POST['datos']['dirección']."',
                '".$_POST['datos']['sexo']."')";
        ...
        //Preparamos los datos
            ...
            $password = password_hash($_POST['datos']['password'], PASSWORD_DEFAULT);
        //
